...
```{r, out.width = "100%"}
# create datatable to explore the data in the barplot
DT::datatable(mapping_rates_data %>%
                dplyr::mutate(across(c(sample,
                                       treatment,
                                       low_fastq_read_count), base::as.factor)) %>%
                dplyr::mutate(across(c(raw_read_count_fastq_file,
                                       ...
                                       smrnaseq_reads_unmapped_to_hairpin_bowtie_stats_file), base::as.integer)) %>%
                dplyr::select(sample,
                              treatment,
...
DT::datatable(mapping_rates_data %>%
                ...
                dplyr::mutate(across(c(raw_read_count_fastq_file,
                                       ...
                                       excerpt_not_mapped_to_genome_or_libs_stats_file), base::as.integer)) %>%
                dplyr::select(sample,
                              treatment,
                              raw_read_count_fastq_file,
                              excerpt_reads_used_for_alignment_stats_file,
