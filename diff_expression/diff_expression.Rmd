...
```{r, eval = config$mirna_smrnaseq, out.width = "100%"}
summary(decideTests(mirna_smrnaseq_limmavoom_efit)) %>%
  base::as.data.frame() %>%
...
```{r, eval = config$mirna_excerpt, out.width = "100%"}
summary(decideTests(mirna_excerpt_limmavoom_efit)) %>%
  base::as.data.frame() %>%
  dplyr::select(Var1, Var2, Freq) %>%
  DT::datatable(
          filter = "top",
...
```{r, eval = config$pirna_excerpt, out.width = "100%"}
summary(decideTests(pirna_excerpt_limmavoom_efit)) %>%
  base::as.data.frame() %>%
  dplyr::select(Var1, Var2, Freq) %>%
  DT::datatable(
          filter = "top",
...
```{r, eval = config$trna_excerpt, out.width = "100%"}
summary(decideTests(trna_excerpt_limmavoom_efit)) %>%
  base::as.data.frame() %>%
  dplyr::select(Var1, Var2, Freq) %>%
  DT::datatable(
          filter = "top",
...
```{r, eval = config$circrna_excerpt, out.width = "100%"}
summary(decideTests(circrna_excerpt_limmavoom_efit)) %>%
  base::as.data.frame() %>%
  dplyr::select(Var1, Var2, Freq) %>%
  DT::datatable(
          filter = "top",
...
```{r, eval = config$gencode_excerpt, out.width = "100%"}
summary(decideTests(gencode_excerpt_limmavoom_efit)) %>%
  base::as.data.frame() %>%
  dplyr::select(Var1, Var2, Freq) %>%
  DT::datatable(
          filter = "top",
...
```{r, eval = config$mirna_smrnaseq, out.width = "100%"}
mirna_smrnaseq_limmavoom_results <- decideTests(mirna_smrnaseq_limmavoom_efit)
mfrow.old <- par()$mfrow
vennDiagram(mirna_smrnaseq_limmavoom_results, 
    include=c("up", "down"),
...
```{r, eval = config$mirna_excerpt, out.width = "100%"}
mirna_excerpt_limmavoom_results <- decideTests(mirna_excerpt_limmavoom_efit)
mfrow.old <- par()$mfrow
vennDiagram(mirna_excerpt_limmavoom_results, 
    include=c("up", "down"),
    counts.col=c("red", "blue"),
    circle.col = c("red", "blue", "green3"),
    cex = 1)
...
```{r, eval = config$pirna_excerpt, out.width = "100%"}
pirna_excerpt_limmavoom_results <- decideTests(pirna_excerpt_limmavoom_efit)
mfrow.old <- par()$mfrow
vennDiagram(pirna_excerpt_limmavoom_results, 
    include=c("up", "down"),
    counts.col=c("red", "blue"),
    circle.col = c("red", "blue", "green3"),
    cex = 1)
...
```{r, eval = config$trna_excerpt, out.width = "100%"}
trna_excerpt_limmavoom_results <- decideTests(trna_excerpt_limmavoom_efit)
mfrow.old <- par()$mfrow
vennDiagram(trna_excerpt_limmavoom_results, 
    include=c("up", "down"),
    counts.col=c("red", "blue"),
    circle.col = c("red", "blue", "green3"),
    cex = 1)
...
```{r, eval = config$circrna_excerpt, out.width = "100%"}
circrna_excerpt_limmavoom_results <- decideTests(circrna_excerpt_limmavoom_efit)
mfrow.old <- par()$mfrow
vennDiagram(circrna_excerpt_limmavoom_results, 
    include=c("up", "down"),
    counts.col=c("red", "blue"),
    circle.col = c("red", "blue", "green3"),
    cex = 1)
...
```{r, eval = config$gencode_excerpt, out.width = "100%"}
gencode_excerpt_limmavoom_results <- decideTests(gencode_excerpt_limmavoom_efit)
mfrow.old <- par()$mfrow
vennDiagram(gencode_excerpt_limmavoom_results, 
    include=c("up", "down"),
    counts.col=c("red", "blue"),
    circle.col = c("red", "blue", "green3"),
    cex = 1)
...
datatable(sig_diff_expr_results %>%
            ...
            dplyr::select(rna,
                          ...
                          adj_p_value,
                          significance) %>%
            dplyr::mutate(across(c(rna_species, pipeline, diff_expr_method, comparison, significance), as.factor)),
          filter = "top",
          rownames = FALSE,
          colnames = c("RNA",
                       "RNA species",
