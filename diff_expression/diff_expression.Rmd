...
keep <- base::lapply(dge, function(x) {
  ...
  filterByExpr(x, group=ordered_treatments)

})

# dge_filtered <- base::lapply(dge, function(x) {
#   
#   x[keep$x, keep.lib.sizes=FALSE]
# 
# })

# get a list of RNA's that were filtered out (are FALSE in the the keep_exprs lists)
rnas_filtered <- base::lapply(keep, function(x) {
...
norm_factors <- base::lapply(dge, function(x) {

  edgeR::calcNormFactors(x, method="TMM")
...
mirna_smrnaseq_limmavoom_v <- voom(dge$mirna_smrnaseq,  model_matrix$mirna_smrnaseq, plot=TRUE)
```
...
# remove heteroscedascity from count data and plot mean variance
mirna_excerpt_limmavoom_v <-voom(dge$mirna_excerpt,  model_matrix$mirna_excerpt, plot=TRUE)
```
...
# remove heteroscedascity from count data and plot mean variance
pirna_excerpt_limmavoom_v <-voom(dge$pirna_excerpt,  model_matrix$pirna_excerpt, plot=TRUE)
```
...
```{r, eval = config$trna_excerpt, results = "asis"}
# remove heteroscedascity from count data and plot mean variance
trna_excerpt_limmavoom_v <-voom(dge$trna_excerpt,  model_matrix$trna_excerpt, plot=TRUE)
```
...
# remove heteroscedascity from count data and plot mean variance
circrna_excerpt_limmavoom_v <-voom(dge$circrna_excerpt,  model_matrix$circrna_excerpt, plot=TRUE)
```
...
# remove heteroscedascity from count data and plot mean variance
gencodes_excerpt_limmavoom_v <-voom(dge$gencode_excerpt,  model_matrix$gencodes_excerpt, plot=TRUE)
```
