...
```{r, results = "asis"}
base::cat(base::paste0("RNA's found to be significantly differentially expressed (with an adjusted p-value less than ", config$pvalue_cutoff, " and a log fold change greater than ", config$min_lfc ,") by the [limma/voom](https://genomebiology.biomedcentral.com/articles/10.1186/gb-2014-15-2-r29) differential expression method are reported here."))
```
...
```{r, results = "asis"}
base::cat(base::paste0("This differential expression method is undertaken using the [DESeq2](https://www.rdocumentation.org/packages/DESeq2/versions/1.12.3/topics/DESeq) function"))
```
...
base::cat("These plots are created using the [plotMA](https://www.rdocumentation.org/packages/DESeq2/versions/1.12.3/topics/plotMA) function which plots the log2 fold changes versus the mean of normalized counts using the p-value cutoff of ", config$pvalue_cutoff, ".", "These can be used to visualize the differences between treatment group comparisons. In general we would expect the expression of genes to remain consistent between treatment groups and so the MA plot should be similar to the shape of a trumpet with most points residing on a y intercept of 0. Differentially expressed RNA's are marked blue.")
```
...
```{r, results = "asis"}
base::cat(base::paste0("RNA's found to be significantly differentially expressed (with an adjusted p-value less than ", config$pvalue_cutoff, " and a log fold change greater than ", config$min_lfc ,") by the [deseq2](https://genomebiology.biomedcentral.com/articles/10.1186/s13059-014-0550-8) differential expression method are reported here."))
```
...
```{r, results = "asis"}
base::cat(base::paste0("RNA's found to be significantly differentially expressed (with an adjusted p-value less than ", config$pvalue_cutoff, " and a log fold change greater than ", config$min_lfc ,") by both the [limma/voom](https://genomebiology.biomedcentral.com/articles/10.1186/gb-2014-15-2-r29) and the [deseq2](https://genomebiology.biomedcentral.com/articles/10.1186/s13059-014-0550-8) differential expression method are reported here."))
```
