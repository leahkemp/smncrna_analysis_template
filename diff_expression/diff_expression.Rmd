...
# remove heteroscedascity from count data and plot mean variance
gencodes_excerpt_limmavoom_v <- limma::voom(dge_gencode_excerpt_filtered,  model_matrix$gencodes_excerpt, plot=TRUE)
```
...
limma_voom_results_1 <- limma_voom_results_1 %>% 
  dplyr::rename_at(vars(-c(rna, rna_species, pipeline, diff_expr_method, comparison)), function(x) base::paste0(x, "_limma_voom"))
...
# (eg. that padj was from a deseq2 analysis and adj.P.Val was from a limma voom analysis)
deseq_results_1 <- deseq_results_1 %>% 
  dplyr::rename_at(vars(-c(rna, rna_species, pipeline, diff_expr_method, comparison)), function(x) base::paste0(x, "_deseq2"))

##### both limma/voom and deseq #####

# collapse results from deseq and limma/voom differential expression analysis together
diff_expr_results_1 <- dplyr::full_join(limma_voom_results_1, deseq_results_1)
```
...
# collapse results from deseq and limma/voom differential expression analysis together
diff_expr_results_2 <- dplyr::full_join(limma_voom_results_2, deseq_results_2)
```
...
# collapse results from deseq and limma/voom differential expression analysis together
diff_expr_results_3 <- dplyr::full_join(limma_voom_results_3, deseq_results_3)
```
...
# collapse results from deseq and limma/voom differential expression analysis together
diff_expr_results_4 <- dplyr::full_join(limma_voom_results_4, deseq_results_4)
```
...
# collapse results from deseq and limma/voom differential expression analysis together
diff_expr_results_5 <- dplyr::full_join(limma_voom_results_5, deseq_results_5)
```
...
# collapse results from deseq and limma/voom differential expression analysis together
diff_expr_results_6 <- dplyr::full_join(limma_voom_results_6, deseq_results_6)
```
...
diff_expr_results <- diff_expr_results %>%
  dplyr::mutate(significance = dplyr::case_when((adj.P.Val_limma_voom > 0.10) | (padj_deseq2 > 0.10) ~ "not_significant",
...
datatable(diff_expr_results %>%
          ...
          options = base::list(
            dom = "BRrltpi",
            autoWidth = FALSE,
            columnDefs = base::list(base::list(width = "150px", targets = 1)),
