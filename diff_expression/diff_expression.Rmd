...
mirna_excerpt_data <- utils::read.table(file.path(config$template_dir,
...
pirna_excerpt_data <- utils::read.table(file.path(config$template_dir,
...
trna_excerpt_data <- utils::read.table(file.path(config$template_dir,
...
circrna_excerpt_data <- utils::read.table(file.path(config$template_dir,
...
gencode_excerpt_data <- utils::read.table(file.path(config$template_dir,
...
mirna_smrnaseq_data <- utils::read.table(file.path(config$template_dir,
...
contrasts <- append(config$contrasts, rep.int(FALSE, times = 6))
contrasts <- contrasts[1:6]
contrasts[contrasts != FALSE] <- TRUE
contrasts <- as.logical(contrasts)
...
base::cat(paste0(" - ", config$contrasts, "\n"))
```
...
base::cat(paste0("Filtering settings:"))
base::cat(paste0("- Min total count:", config$min_total_count, "\n", " - Min count:", config$min_count, "\n"))
```
...
keep <- base::lapply(dge, function(x) {
  ...
  filterByExpr(x, group=ordered_treatments, min.count = config$min_count, min.total.count = config$min_total_count)
...
mirna_smrnaseq_limmavoom_v <- voom(dge_mirna_smrnaseq_filtered,  model_matrix$mirna_smrnaseq, plot=TRUE)
```
...
# remove heteroscedascity from count data and plot mean variance
mirna_excerpt_limmavoom_v <- voom(dge_mirna_excerpt_filtered,  model_matrix$mirna_excerpt, plot=TRUE)
```
...
# remove heteroscedascity from count data and plot mean variance
pirna_excerpt_limmavoom_v <- voom(dge_pirna_excerpt_filtered,  model_matrix$pirna_excerpt, plot=TRUE)
```
...
# remove heteroscedascity from count data and plot mean variance
trna_excerpt_limmavoom_v <- voom(dge_trna_excerpt_filtered,  model_matrix$trna_excerpt, plot=TRUE)
```
...
# remove heteroscedascity from count data and plot mean variance
circrna_excerpt_limmavoom_v <- voom(dge_circrna_excerpt_filtered,  model_matrix$circrna_excerpt, plot=TRUE)
```
...
# remove heteroscedascity from count data and plot mean variance
gencodes_excerpt_limmavoom_v <- voom(dge_gencode_excerpt_filtered,  model_matrix$gencodes_excerpt, plot=TRUE)
```
...
```{r, results = "asis"}
# optionally include/print markdown header for this section if being analysed
base::cat(paste0("- The residual variances plots present data only for the first treatment group comparison/contrast: ", config$contrasts[1]))
```
...
mirna_smrnaseq_limmavoom_vfit <- limma::contrasts.fit(mirna_smrnaseq_limmavoom_vfit, coefficients = 1)
...
y <- sqrt(mirna_smrnaseq_limmavoom_efit$sigma)
...
data %>%
  plotly::plot_ly(x = ~x,
                  y = ~y,
                  hoverinfo = "text",
                  text = ~ base::paste("</br> Average log expression:", format(x, big.mark = ",", scientific = FALSE, digits = 2),
                                       "</br> sqrt(sigma):", format(y, big.mark = ",", scientific = FALSE, digits = 2),
                                       "</br> RNA:", rna)) %>%
...
# estimate contrast for each miRNA
mirna_excerpt_limmavoom_vfit <- limma::contrasts.fit(mirna_excerpt_limmavoom_vfit, coefficients = 1)
...
y <- sqrt(mirna_excerpt_limmavoom_efit$sigma)
...
# estimate contrast for each miRNA
pirna_excerpt_limmavoom_vfit <- limma::contrasts.fit(pirna_excerpt_limmavoom_vfit, coefficients = 1)
...
y <- sqrt(pirna_excerpt_limmavoom_efit$sigma)
...
# estimate contrast for each miRNA
trna_excerpt_limmavoom_vfit <- limma::contrasts.fit(trna_excerpt_limmavoom_vfit, coefficients = 1)
...
y <- sqrt(trna_excerpt_limmavoom_efit$sigma)
...
# estimate contrast for each miRNA
circrna_excerpt_limmavoom_vfit <- limma::contrasts.fit(circrna_excerpt_limmavoom_vfit, coefficients = 1)
...
y <- sqrt(circrna_excerpt_limmavoom_efit$sigma)
...
# estimate contrast for each miRNA
gencode_excerpt_limmavoom_vfit <- limma::contrasts.fit(gencode_excerpt_limmavoom_vfit, coefficients = 1)
...
y <- sqrt(gencode_excerpt_limmavoom_efit$sigma)
...
deseq_results_1 <- base::lapply(deseq, function(x) {
  ...
  x %>%
    DESeq2::results(contrast=c("treatment", sub(" - .*", "", config$contrasts[1]), sub(".* - ", "", config$contrasts[1])))
...
deseq_results_2 <- base::lapply(deseq, function(x) {
  ...
  x %>%
    DESeq2::results(contrast=c("treatment", sub(" - .*", "", config$contrasts[2]), sub(".* - ", "", config$contrasts[2])))
...
deseq_results_3 <- base::lapply(deseq, function(x) {
  ...
  x %>%
    DESeq2::results(contrast=c("treatment", sub(" - .*", "", config$contrasts[3]), sub(".* - ", "", config$contrasts[3])))
...
deseq_results_4 <- base::lapply(deseq, function(x) {
  ...
  x %>%
    DESeq2::results(contrast=c("treatment", sub(" - .*", "", config$contrasts[4]), sub(".* - ", "", config$contrasts[4])))
...
deseq_results_5 <- base::lapply(deseq, function(x) {
  ...
  x %>%
    DESeq2::results(contrast=c("treatment", sub(" - .*", "", config$contrasts[5]), sub(".* - ", "", config$contrasts[5])))
...
deseq_results_6 <- base::lapply(deseq, function(x) {
  ...
  x %>%
    DESeq2::results(contrast=c("treatment", sub(" - .*", "", config$contrasts[6]), sub(".* - ", "", config$contrasts[6])))
...
```{r, results = "asis"}
# optionally include/print markdown header for this section if being analysed
base::cat(paste0("- The count outlier detection plots present data only for the first treatment group comparison/contrast: ", config$contrasts[1]))
```
...
```{r, results = "asis"}
# optionally include/print markdown header for this section if being analysed
base::cat(paste0("- The MA plots present data only for the first treatment group comparison/contrast: ", config$contrasts[1]))
```
...
limma_voom_results_1 <- do.call(rbind, limma_voom_results_1)
...
# define the source of the statistics (column names) before join
# (eg. that padj was from a deseq2 analysis and adj.P.Val was from a limma voom analysis)
limma_voom_results_1 <- limma_voom_results_1 %>% 
  rename_at(vars(-c(rna, rna_species, pipeline, diff_expr_method, comparison)), function(x) paste0(x, "_limma_voom"))
...
# (eg. that padj was from a deseq2 analysis and adj.P.Val was from a limma voom analysis)
deseq_results_1 <- deseq_results_1 %>% 
  rename_at(vars(-c(rna, rna_species, pipeline, diff_expr_method, comparison)), function(x) paste0(x, "_deseq2"))

##### both limma/voom and deseq #####

# collapse results from deseq and limma/voom differential expression analysis together
diff_expr_results_1 <- full_join(limma_voom_results_1, deseq_results_1)
```

```{r, contrast2, eval = contrasts[2]}
##### limma/voom #####
...
# collapse the list of dataframes into one
limma_voom_results_2 <- do.call(rbind, limma_voom_results_2)
...
# collapse results from deseq and limma/voom differential expression analysis together
diff_expr_results_2 <- full_join(limma_voom_results_2, deseq_results_2)
```

```{r, contrast3, eval = contrasts[3]}
##### limma/voom #####
...
# collapse the list of dataframes into one
limma_voom_results_3 <- do.call(rbind, limma_voom_results_3)
...
# collapse results from deseq and limma/voom differential expression analysis together
diff_expr_results_3 <- full_join(limma_voom_results_3, deseq_results_3)
```

```{r, contrast4, eval = contrasts[4]}
##### limma/voom #####
...
# collapse the list of dataframes into one
limma_voom_results_4 <- do.call(rbind, limma_voom_results_4)
...
# collapse results from deseq and limma/voom differential expression analysis together
diff_expr_results_4 <- full_join(limma_voom_results_4, deseq_results_4)
```

```{r, contrast5, eval = contrasts[5]}
##### limma/voom #####
...
# collapse the list of dataframes into one
limma_voom_results_5 <- do.call(rbind, limma_voom_results_5)
...
# collapse results from deseq and limma/voom differential expression analysis together
diff_expr_results_5 <- full_join(limma_voom_results_5, deseq_results_5)
```

```{r, contrast6, eval = contrasts[6]}
##### limma/voom #####
...
# collapse the list of dataframes into one
limma_voom_results_6 <- do.call(rbind, limma_voom_results_6)
...
# collapse results from deseq and limma/voom differential expression analysis together
diff_expr_results_6 <- full_join(limma_voom_results_6, deseq_results_6)
```
...
diff_expr_results  <- do.call(rbind, diff_expr_results)
...
diff_expr_results <- diff_expr_results %>%
  mutate(significance = case_when((adj.P.Val_limma_voom > 0.10) | (padj_deseq2 > 0.10) ~ "not_significant",
                                  ((adj.P.Val_limma_voom > 0.05) & (adj.P.Val_limma_voom <= 0.10)) | ((padj_deseq2 > 0.05) & (padj_deseq2 <= 0.10)) ~ "significant_10%",
                                  ((adj.P.Val_limma_voom > 0.01) & (adj.P.Val_limma_voom <= 0.05)) | ((padj_deseq2 > 0.01) & (padj_deseq2 <= 0.05)) ~ "significant_5%",
                                  (adj.P.Val_limma_voom < 0.01) | (padj_deseq2 < 0.01) ~ "significant_1%"))

# then replace any NA's with "not_significant"
diff_expr_results <- diff_expr_results %>%
  dplyr::mutate_at(vars(significance), ~replace(., is.na(.), "not_significant"))

# since there is a row for each differential expresion method (for each RNA and pipeline)...
# make one column for log fold change and adjusted p-value
diff_expr_results <- diff_expr_results %>% 
  tidyr::unite(log_fc, c("logFC_limma_voom", "log2FoldChange_deseq2"), na.rm = TRUE) %>% 
  tidyr::unite(adj_p_value, c("adj.P.Val_limma_voom", "padj_deseq2"), na.rm = TRUE)
...
diff_expr_results %>%
  write.table(file = "./diff_expr_results/diff_expr_results.tsv", row.names=FALSE, sep="\t")
```
