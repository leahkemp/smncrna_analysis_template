...
volcano_interactive <- function(data_frame) {
  ...
  plotly::plot_ly(data = data_frame,
    ...
    add_trace(data = dplyr::filter(data_frame, (adj_p_value <= 0.01) & ((log_fc >= min_logfc) | (log_fc <= -min_logfc))),
              ...
              y = ~adj_p_value,
              type = "scatter",
              marker = list(color = "#FFD500", opacity = 1)) %>%
...
*Note. some values are rounded with the [round function](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/Round)*
...
```{r, results = "asis"}
base::cat(base::paste0("RNA's with a **p-value** smaller than 0.1 and a log fold change greater than ", config$min_lfc, " or a log fold change smaller than -", config$min_lfc, " are plotted here."))
```

```{r, results = "asis"}
base::cat(base::paste0("- Grey: RNA's with an **adjusted p-value** greater than of 0.1 and a log fold change greater than ", config$min_lfc, " or a log fold change smaller than -", config$min_lfc), ". These RNA's are marked as 'greater_than_10% in the tables.'")
```

```{r, results = "asis"}
base::cat(base::paste0("- Red: RNA's with an **adjusted p-value** smaller than of 0.1 and a log fold change greater than ", config$min_lfc, " or a log fold change smaller than -", config$min_lfc), ". These RNA's are marked as 'significant_10% in the tables.'")
```

```{r, results = "asis"}
base::cat(base::paste0("- Orange: RNA's with an **adjusted p-value** smaller than of 0.05 and a log fold change greater than ", config$min_lfc, " or a log fold change smaller than -", config$min_lfc), ". These RNA's are marked as 'significant_5% in the tables.'")
```

```{r, results = "asis"}
base::cat(base::paste0("- Yellow: RNA's with an **adjusted p-value** smaller than of 0.01 and a log fold change greater than ", config$min_lfc, " or a log fold change smaller than -", config$min_lfc), ". These RNA's are marked as 'significant_1% in the tables.'")
```
...
```{r, results = "asis"}
base::cat(base::paste0("RNA's with a **p-value** smaller than 0.1 and a log fold change greater than ", config$min_lfc, " or a log fold change smaller than -", config$min_lfc, " for the [limma/voom](https://genomebiology.biomedcentral.com/articles/10.1186/gb-2014-15-2-r29) differential expression method are reported here."))
```
...
```{r, results = "asis"}
base::cat(base::paste0(" - 'greater_than_10%' means the RNA meets the logFC threshold and has an **adjusted p-value** greater than 0.10"))
```
...
```{r, results = "asis"}
base::cat(base::paste0("RNA's with a **p-value** smaller than 0.1 and a log fold change greater than ", config$min_lfc, " or a log fold change smaller than -", config$min_lfc, " for the [deseq2](https://genomebiology.biomedcentral.com/articles/10.1186/s13059-014-0550-8) differential expression method are reported here."))
```
