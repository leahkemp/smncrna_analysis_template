...
- *For each combination of sample, treatment and pipeline, the total number of RNA counts are calculated for each RNA species, then the average of this is taken for each treatment group to calculate the average raw count ('Average raw count') in a given sample, for each combination of treatment and pipeline*
- *Then for each combination of treatment and pipeline, the average total RNA's identified are calculated. This is then used to calculate 'Percentage of average number of all RNA's identified' in the given treatment and pipeline*
