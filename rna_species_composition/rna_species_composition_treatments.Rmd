...
- *For each combination of sample, treatment and pipeline, the total number of RNA counts are calculated for each RNA species*
- *This is then used to calculate the average raw count ('Average raw count') in a given sample, for each combination of treatment and pipeline*
- *Then for each combination of treatment and pipeline, the total RNA's identified are calculated. This is then used to calculate 'Percentage of all RNA's identified' in the given treatment and pipeline*
...
total_rna_abundance  <- counts %>%
  dplyr::group_by(treatment, pipeline) %>%
  dplyr::summarise(total_raw_count_all_rnas = sum(raw_counts))
...
data <- data %>%
  dplyr::mutate(percentage_total_raw_count_all_rnas = base::round((mean_total_raw_count/total_raw_count_all_rnas)*100, digits = 2))

# create datatable of data
data %>%
  base::as.data.frame() %>%
  dplyr::select(treatment, rna_species_2, pipeline, mean_total_raw_count, percentage_total_raw_count_all_rnas) %>%
